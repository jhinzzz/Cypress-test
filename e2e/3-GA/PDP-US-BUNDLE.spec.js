...
describe("test PDP GTM", () => {
  ...
  before("product information prepare", () => {
    ...
    cy
      .request("GET", Cypress.env("US_API") + "/v3" + Cypress.env("US_BUN"))
      .then((response) => 
      {
        //获取产品信息
        GA.productSKU = response.body.variants[0].sku;  //修改bundle后需要修改
        GA.productName = response.body.variants[0].name;  //修改bundle后需要修改
        GA.productLabel = GA.productSKU + " | " + GA.productName;
        // 找到包含table option数据的数组并返回
        GA.productLength = response.body.bundle_options[0].option_types[0].values[1].presentation
        GA.productSwitchLength = response.body.bundle_options[0].option_types[0].values[0].presentation
        GA.productOptName = response.body.bundle_options[0].option_types.name
  ...
  it("test US bundle", () => {
    ...
    cy.visit(Cypress.env("US_HPG") + Cypress.env("US_BUN")).then(() =>{
      // 触发PDP_image
      PDP.getImage().click();
      // 触发wish_list
