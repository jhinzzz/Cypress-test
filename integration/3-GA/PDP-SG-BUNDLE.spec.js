...
describe("test PDP GTM", () => {
  ...
  it("test SG bundle", () => {
    ...
    /*---------------------------------------------触发GTM---------------------------------------------*/
    cy.visit(Cypress.env("SG_HPG") + Cypress.env("SG_BUN"));
    ...
    cy.get("h4").contains("Vincent Dining Table, Walnut").click().then(()=>{
        cy.get("button").contains("180cm").click().then(()=>{
    ...
    cy.window().then((win) => {
      win.dataLayer.some((dl) => {
        ...
        if (
          ...
          dl["eventDetails.label"] === product_length &&
          dl["eventDetails.sku_id"] === product_sku
        ) {
            options_res = true;
        }
      ...
      expect(options_res, "is PDP has a 180cm options").to.equal(true);                 // 断言options_change
