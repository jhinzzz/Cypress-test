...
let options_res = false;            // options_change断言结果
...
describe("test PDP GTM", () => {
  ...
  before("product imformation prepared", () => {
    ...
    cy
      .request("GET", Cypress.env("US_API") + "/v2" + Cypress.env("US_BUN"))
      .then((response) => {
        product_sku = response.body.variants[0].sku;
        product_name = response.body.variants[0].name;
        product_label = product_sku + " | " + product_name;
        product_option = response.body.bundle_options[0].option_types[1].values[1].presentation
        product_opt_name = response.body.bundle_options[0].option_types[1].name
  ...
  it("test US bundle", () => {
    ...
    cy.get("h3").contains("Product Details").click().then(() => {
        ...
        // 触发details_close  
        cy.get("h3").contains("Product Details").click(); 
    }); 
    // 触发dimensions_expand
    cy.get("h3").contains("Product Dimensions").click().then(()=>{
        // 触发dimensions_close
        cy.get("h3").contains("Product Dimensions").click(); 
    });    
    // 触发delivery_expand
    ...
    cy.window().then((win) => {
      win.dataLayer.some((dl) => {
        ...
        //测试options_change
        if (
          dl["eventDetails.action"] === product_opt_name &&
          dl["eventDetails.label"] === product_option &&
          dl["eventDetails.sku_id"] === product_sku
        ) {
            options_res = true;
        }
        ...
        console.log(pdp_property_res, product_detail_label)
        //测试PDP_installment
        if (
      ...
      expect(options_res, "is PDP has a right face options").to.equal(true);            // 断言options_change
