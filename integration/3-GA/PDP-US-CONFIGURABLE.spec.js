...
let swatch_cart_res = false;        // swatch_cart断言结果
let material_res = false;           // material_change断言结果
...
describe("test PDP GTM", () => {
  ...
  before("product imformation prepared", () => {
    cy
      .request("GET", Cypress.env("US_API") + "v2/" + Cypress.env("US_CON"))
      .then((response) => {
        product_sku = response.body.variants[0].sku;
        product_name = response.body.variants[0].name;
        product_label = product_sku + " | " + product_name;
        product_material = response.body.variants[0].variant_option_values[0].presentation
        product_material_name = response.body.variants[0].variant_option_values[0].option_type_name
  ...
  it("test US configurable", () => {
    ...
    // 触发swatch
    cy.get('[data-selenium="open_swatch"]')
      .click()
      .then(() => {
        cy.get('[data-selenium="add_remove_swatch"]').click({multiple:true}); 
        cy.get('svg[aria-label="dismiss"]').click();
      });
    // 触发material change，后续需要修改标签
    cy.get(".ixioeN").find("button").click({multiple:true}); 
    ...
    cy.get(".affirm-modal-trigger").click().then(() => {
        cy.frameLoaded(".affirm-sandbox-iframe");
        //切换到iframe
        cy.enter(".affirm-sandbox-iframe").then((getBody) => {
          // 触发installment_close
    ...
    cy.wait(1000)
    // 触发details_expand
    cy.get("div").contains("Product Details").click().then(() => {
        // 触发pdp_property，修改product需更新
        cy.get("span").contains(product_detail_value).find("svg").click();
        //关闭property弹窗，后续需要修改标签
        cy.get("div.ZPYEqa__explanationModal").find('svg[aria-label="dismiss"]').click();
        // 触发details_close  
        cy.get("div").contains("Product Details").click(); 
    }); 
    // 触发dimensions_expand
    cy.get("div").contains("Product Dimensions").click().then(()=>{
        // 触发dimensions_close
        cy.get("div").contains("Product Dimensions").click(); 
    });    
    // 触发delivery_expand
    cy.get("h3").contains("Delivery").click().then(()=>{
        // 触发delivery_close
        cy.get("h3").contains("Delivery").click(); 
    }); 
    ...
    cy.window().then((win) => {
      win.dataLayer.some((dl) => {
        ...
        if (
          ...
          dl["eventDetails.label"] === "VL-4008 | Swatch, Midnight Blue Velvet"
        ) {
          swatch_cart_res = true;
        }
        //测试material_change
        if (
          dl["eventDetails.action"] === product_material_name &&
          dl["eventDetails.label"] === product_material &&
          dl["eventDetails.sku_id"] === product_sku
        ) {
          material_res = true;
        }
      ...
      expect(swatch_cart_res, "is PDP has a swatch_cart").to.equal(true);               // 断言swatch_cart
      expect(material_res, "is PDP has a Ash Grey material").to.equal(true);            // 断言material_change
