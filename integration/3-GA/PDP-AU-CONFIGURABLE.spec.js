...
describe("test PDP GTM", () => {
  ...
  /*---------------------------测试主体---------------------------*/
  it("test AU configurable", () => {

    /*---------------------------------------------触发GTM---------------------------------------------*/
    cy.visit(Cypress.env("AU_HPG") + Cypress.env("AU_CON"));
    ...
    cy.get(".ixioeN__container").find("button").click({multiple:true}); 
    // 触发x_reviews，后续需要修改标签
    cy.get("a.rf7vq7__rating-count").click({force:true}); 
    // 触发review_dropdown
    ...
    cy.window().then((win) => {
      win.dataLayer.some((dl) => {
        ...
        if (
          ...
          dl["eventDetails.label"] === "SR-4005 | Swatch, Fumo Grey"
        ) {
      ...
      expect(material_res, "is PDP has a Canary Yellow material").to.equal(true);       // 断言material_change
