...
describe("test PDP GTM", () => {
  ...
  before("product imformation prepared", () => {
    ...
    cy
      .request("GET", Cypress.env("SG_API") + "/v2" + Cypress.env("SG_CON"))
      .then((response) => {
  ...
  it("test SG configurable", () => {
    ...
    cy.get('[data-selenium="open_swatch"]')
      .click()
      .then(() => {
        cy.get('[data-selenium="add_remove_swatch"]').click(); 
        cy.get('svg[aria-label="dismiss"]').click();
      });
    // 触发material change，后续需要修改标签
    cy.get(".ixioeN").find("button").click(); 
    ...
    cy.get('[data-selenium="product-price"]').parent().find("a").click(); 
    // 触发installment_close，后续需要修改标签
    cy.get('div[role="menuitem"]').find('svg[aria-label="dismiss"]').click(); 
    // 触发details_expand
    ...
    cy.window().then((win) => {
      win.dataLayer.some((dl) => {
        ...
        if (
          ...
          dl["eventDetails.label"] === "DU-4005 | Swatch, Ash Grey"
        ) {
